...
Simulating the behavior of a real asset is a core capability of an advanced digital twin, enabling users to test scenarios, analyze system dynamics, and predict future states without affecting the real-world counterpart. OpenTwins offers a flexible approach to integrate these simulations into the platform, relying on direct communication with the platform's message brokers. This support can be composed of one or both of the following elements:

  - [FMI API](https://github.com/ertis-research/opentwins-fmi-2.0). The [Functional Mock-up Interface (FMI)](https://fmi-standard.org/) is a widely adopted standard for exchanging dynamic simulation models. To facilitate its use, we have developed a specific component that allows users to upload FMIs and interact with them via HTTP requests. Once the simulation is executed, this API automatically sends the resulting data to the active message broker (Apache Kafka or Mosquitto), making it available for the digital twin. You can learn more about how to use and deploy this tool in its [specific documentation section](./../guides/fmi/concepts.mdx).

  - *Any custom simulation*. Rather than a specific software component provided by OpenTwins, this represents a flexible integration pattern. The platform supports the integration of any custom simulation that is encapsulated (e.g., within a Docker container), provided it exposes an HTTP interface for interaction and is configured to publish its execution results to Apache Kafka or Mosquitto. This ensures that any proprietary simulation tool, model, or script that does not follow the FMI standard can still be integrated into OpenTwins.
