...
- **Grafana public folder**: Place files in Grafana’s `/public` directory and reference relative paths. For example, on Windows the path is usually: `C:\Program Files\GrafanaLabs\grafana\public`. In containerized environments, you can use an initContainer to copy the files into the `/public` directory before Grafana starts. This makes the assets available at runtime without modifying the base Grafana image.
